...
/// An ambiguous publication could not be resolved safely by readback.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct AmbiguityError {
    /// Publication key whose outcome remains unknown.
    pub key: ObjectKey,
}
...
impl fmt::Display for AmbiguityError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "publication outcome for {} is ambiguous",
            self.key
        )
    }
}
...
pub enum StorageError<E> {
    ...
    /// Unresolved ambiguous publication result.
