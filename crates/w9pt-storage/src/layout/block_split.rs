...
pub(crate) async fn truncate<S: TargetStore>(
    ...
    let old_final_block = if manifest.logical_size() % block_size != 0 {
    ...
    if logical_size < manifest.logical_size() {
        let first_discarded = logical_size / block_size + u64::from(logical_size % block_size != 0);
        ...
        if visible_tail != 0 {
            if let Some(old) = result.get(&final_index).cloned() {
                ...
                let tail = usize::try_from(visible_tail)
