...
struct State {
    ...
    ambiguous_puts_without_commit: Vec<Option<ObjectKey>>,
}
...
impl Default for State {
    fn default() -> Self {
        Self {
            ...
            trace: Vec::new(),
            failures: Vec::new(),
            ambiguous_puts_without_commit: Vec::new(),
        }
    }
}
...
impl State {
    ...
    fn should_leave_put_ambiguous_without_commit(&mut self, key: &ObjectKey) -> bool {
        let Some(index) = self
            .ambiguous_puts_without_commit
            .iter()
            .position(|expected| expected.as_ref().is_none_or(|expected| expected == key))
        else {
            return false;
        };
        self.ambiguous_puts_without_commit.remove(index);
        true
    }
}
...
impl MemoryTarget {
    ...
    /// Injects one ambiguous immutable-put result without creating the object.
    ///
    /// This models response uncertainty where exact readback proves absence; it
    /// does not represent a definite before-dispatch failure.
    pub fn inject_ambiguous_put_without_commit_for(
        &self,
        key: ObjectKey,
    ) -> Result<(), MemoryTargetError> {
        self.lock()?.ambiguous_puts_without_commit.push(Some(key));
        Ok(())
    }
...
impl TargetStore for MemoryTarget {
    ...
    fn put_if_absent(
        ...
        async move {
            ...
            if state.should_leave_put_ambiguous_without_commit(&key) {
...
mod tests {
    ...
    #[test]
    fn immutable_put_before_failure_is_definite_and_after_failure_is_ambiguous() {
        let target = MemoryTarget::new();
        let before_key = key("v1/data/before");
        target
            .inject_failure(TargetOperation::PutIfAbsent, FailureTiming::Before)
            .unwrap();
        assert!(matches!(
            block_on(target.put_if_absent(before_key.clone(), b"before".to_vec())),
            Err(MemoryTargetError::Injected {
                operation: TargetOperation::PutIfAbsent,
                timing: FailureTiming::Before,
            })
        ));
        assert_eq!(block_on(target.get(before_key, 6)).unwrap(), None);

        let after_key = key("v1/data/after");
        target
            .inject_failure(TargetOperation::PutIfAbsent, FailureTiming::After)
            .unwrap();
        assert_eq!(
            block_on(target.put_if_absent(after_key.clone(), b"after".to_vec())).unwrap(),
            PutIfAbsent::Ambiguous
        );
        assert_eq!(
            block_on(target.get(after_key, 5)).unwrap().unwrap().bytes(),
            b"after"
        );
    }
}
