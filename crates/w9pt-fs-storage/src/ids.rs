...
impl PreparationIdentity {
    ...
    /// Derives the identity of a positioned first write to an unpublished file.
    pub fn for_write_from_new(
        mutation_id: MutationId,
        method: StorageMethod,
        offset: u64,
        bytes: &[u8],
    ...
    /// Derives the identity of an initial truncate against an unpublished file.
    pub fn for_truncate_from_new(
        mutation_id: MutationId,
        method: StorageMethod,
        logical_size: u64,
    ) -> Self {
...
fn fingerprint(parts: &[&[u8]]) -> OperationFingerprint {
    ...
    hasher.update(b"w9pt-fs-storage-operation-v1\0");
    for part in parts {
